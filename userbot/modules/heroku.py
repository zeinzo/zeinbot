...
@zein_ubot(outgoing=True, pattern=r"^\.(get|del) var(?: |$)(\w*)")
...
@zein_ubot(outgoing=True, pattern=r"^\.set var (\w*) ([\s\S]*)")
...
@zein_ubot(outgoing=True, pattern=r"^\.usage$")
...
@zein_ubot(outgoing=True, pattern=r"^\.logs")
async def _(dyno):
    if app is None:
        return await dyno.edit(
            "**Please setup your** `HEROKU_APP_NAME` **and** `HEROKU_API_KEY`**.**"
