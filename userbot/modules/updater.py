...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        repo.config_writer().set_value("user", "name", "zeinzo").release()
        repo.config_writer().set_value("user", "email", "knsgnwn.10@gmail.com").release()
...
@zein_ubot(outgoing=True, pattern=r"^\.update( now| deploy|$)")
